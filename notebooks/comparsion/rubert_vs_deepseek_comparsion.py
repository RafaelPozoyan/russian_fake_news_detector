...
ds_df = pd.read_csv("models/deepseek/predictions_441_v4.csv")
...
for ax, (name, y_t, y_p, m) in zip(axes, [
...
fig.suptitle("Матрицы ошибок: RuBERT и DeepSeek",
...
ax.plot(fpr_rb, tpr_rb, color=RB_COLOR, linewidth=2.2,
...
ax.scatter([fpr_pt], [tpr_pt], color=DS_COLOR, s=240, marker="*",
...
ax.set_title("ROC-кривые: RuBERT и DeepSeek\n"
...
axes[0].hist([rb_prob[correct_rb], rb_prob[~correct_rb]],
             bins=25, stacked=True,
             color=["#3FA34D", "#D7263D"], alpha=0.85,
...
for i, (rb_v, ds_v) in enumerate(zip([rb_acc_fake, rb_acc_real],
                                     [ds_acc_fake, ds_acc_real])):
...
fig.suptitle("Уверенность RuBERT и пер-классовая точность DeepSeek",
             fontsize=14, fontweight="bold", y=1.02)
plt.tight_layout()
...
sns.heatmap(matrix, annot=True, fmt="d", cmap="Greens", ax=axes[0],
...
fig.suptitle("Анализ согласованности: RuBERT и DeepSeek",
             fontsize=14, fontweight="bold", y=1.02)
plt.tight_layout()
