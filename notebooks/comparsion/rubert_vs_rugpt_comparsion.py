...
for ax, (name, y_pred, m) in zip(axes, [
    ("RuBERT", rb_pred, rb_m),
...
fig.suptitle("Матрицы ошибок: RuBERT и ruGPT-3 + LoRA", fontsize=14, fontweight="bold", y=1.02)
plt.tight_layout()
...
ax.plot(fpr_rg, tpr_rg, color=RG_COLOR, linewidth=2.2,
...
axes[1].hist([rg_prob[correct_rg], rg_prob[~correct_rg]],
             bins=25, stacked=True,
             color=["#3FA34D", "#D7263D"], alpha=0.85,
...
fig.suptitle("Уверенность и калибровка предсказаний", fontsize=14, fontweight="bold", y=1.02)
plt.tight_layout()
...
sns.heatmap(matrix, annot=True, fmt="d", cmap="Greens", ax=axes[0],
            ...
            annot_kws={"size": 14}, cbar=False)
axes[0].set_title(
    f"Согласие моделей: {agree}/{N} ({agree/N:.1%})",
    fontsize=11
)
...
fig.suptitle("Анализ согласованности: RuBERT и ruGPT-3 + LoRA",
